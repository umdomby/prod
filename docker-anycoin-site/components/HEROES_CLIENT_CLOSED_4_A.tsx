...
export const HEROES_CLIENT_CLOSED_4_A: React.FC<Props> = ({ user, closedBets }) => {
    const totalProfitLoss = closedBets.reduce((total, bet) => {
        //const userBets = bet.participantsCLOSED4.filter((p) => p.userId === user.id);
        return total + bet.participantsCLOSED4.reduce((sum, p) => sum + (p.isWinner ? p.profit : (p.return - p.amount)), 0);
    }, 0);
                    ...
                    <Accordion type="single" collapsible>
                        <AccordionItem value={`item-${bet.id}`}>
                                        ...
                                        <TableRow>
                                            ...
                                            <TableCell className="text-ellipsis overflow-hidden whitespace-nowrap w-[25%]">
                                                {[bet.player1, bet.player2, bet.player3, bet.player4].map((player, index) => {
                                                    ...
                                                    const playerProfitLoss = bet.participantsCLOSED4
                            ...
                            <AccordionContent>
                                ...
                                {bet.participantsCLOSED4.length > 0 && (
                                    <div className="m-1 p-4 rounded-lg">
                                        <h4 className="text-md font-semibold mb-2">Ставки на этот матч:</h4>
                                        {bet.participantsCLOSED4.map((participant) => {
                                            const profitToCover =
                                            ...
                                            return (
                                                <div key={participant.id} className="border border-gray-200 p-1 mb-1 rounded-md">
                                                    <p>
                                                        ...
                                                        <Link className="text-blue-500 cursor-pointer hover:text-green-500" href={`/users-a-4/${participant.user.id}`}>

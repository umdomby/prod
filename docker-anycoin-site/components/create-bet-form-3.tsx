...
import {Category, Product, ProductItem, User, Player, TurnirBet, Bet3} from '@prisma/client';
import {clientCreateBet3, editBet3} from "@/app/actions"; // Assuming you have a separate action for 3 players
...
interface Props {
    ...
    openBets: Bet3[]; // Добавлено свойство для открытых ставок
}

export const CreateBetForm3: React.FC<Props> = ({ user, categories, products, productItems, players, turnirBet, createBet3, openBets }) => {
    ...
    const [openBetsState, setOpenBets] = useState<Bet3[]>(openBets);
    const [tempChanges, setTempChanges] = useState<{ [key: number]: Partial<Bet3> }>({});
